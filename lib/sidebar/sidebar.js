...
SidebarView.prototype.get = function(index) {
...
SidebarView.prototype.next = function(law) {
  var i = this.index(law);
  if (i == lawsFilter.items().length) return null;
  return this.get(i + 1);
};

SidebarView.prototype.previous = function(law) {
  var i = this.index(law);
  if (i == 0) return null;
  return this.get(i - 1);
};

SidebarView.prototype.index = function(law) {
  for (var i = 0; i < lawsFilter.items().length; i++) {
    if (law.id == lawsFilter.items()[i].id) {
      return i;
    }
  };
  return -1;
