...
app.get('/law/all', function (req, res) {
  ...
  api.law.all(function(err, lawDocs) {
    ...
    var keys = [
      'id lawId mediaTitle summary status open closed public draft deleted',
      'tag participants voted createdAt updatedAt closingAt upvotes downvotes abstentions',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl picture votable backgroundColor fontColor'
    ].join(' ');

    res.json(lawDocs.map(function (lawDoc) { return lawDoc.toJSON() }).map(expose(keys)));
  });
});
...
app.get('/law/:id', function (req, res) {
  ...
  api.law.get(req.params.id, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl picture backgroundColor fontColor'
    ].join(' ');
  ...
  api.law.get(req.params.id, function (err, law) {
    ...
    api.comment.getFor({context: 'summary', reference: re}, null, function (err, comments) {
      ...
      var keys = [
        ...
        'flags upvotes downvotes votes replies.length picture'
      ].join(' ');
...
app.post('/law/:id/comment', restrict, function (req, res) {
  ...
  api.law.comment(comment, function (err, commentDoc) {
    ...
    var keys = [
      ...
      'createdAt replies context reference picture'
    ].join(' ');
...
app.post('/law/create', restrict, staff, function (req, res, next) {
  ...
  api.law.create(req.body, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl picture'
    ].join(' ');
