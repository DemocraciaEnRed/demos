...
/**
 * Law Score Schema
 * - How much do you care about the law?
 */

var Score = new Schema({
    author: { type: ObjectId, ref: 'Citizen', required: true }
  , value: { type: String, enum: ["nothing", "little", "somewhat", "very"], required: true }
  , createdAt: { type: Date, default: Date.now }
});
...
var LawSchema = new Schema({
  ...
  , phaseTwo: { type: Boolean, required: true, default: false }
  , intro: { type: ObjectId, ref: 'Law' }
  , phaseOne: { type: ObjectId, ref: 'Law' }
  , winner: { type: Boolean, required: true, default: false }
  , clauseTruncationText: { type: String }
  , links: [LinkSchema]
  , party: { type: String }
  , partyUrl: { type: String }
  , backgroundColor: { type: String, default: 'transparent' }
  , fontColor: { type: String, default: '#000' }
  , picture: { type: String }
  , scores: [Score]
  , scoreParticipants: [{type: ObjectId, ref: 'Citizen' }]
  , singular: { type: Boolean, required: true, default: false }
  , totalScore: { type: Number }
  , totalNothings: { type: Number }
  , totalLittles: { type: Number }
  , totalSomewhats: { type: Number }
  , totalVerys: { type: Number }
  , scoreCensus: { type: Number }
  , order: { type: Number, required: true, default: 0 }
});
...
LawSchema.index({ deletedAt: -1 });
LawSchema.index({ phaseOne: -1 });
LawSchema.index({ phaseTwo: -1 });
LawSchema.index({ totalScore: -1 });
LawSchema.index({ winner: -1 });
LawSchema.index({ closingAt: -1 });
LawSchema.index({ participants: -1 });
LawSchema.index({ scoreParticipants: -1 });
...
LawSchema.options.toJSON.transform = function(doc, ret, options) {
  ...
  /**
   * Map scores into their respective values
   */
  if (ret.scores) delete ret.scores;
  if (ret.nothings) ret.nothings = ret.nothings.map(function(v) { return v.author });
  if (ret.littles) ret.littles = ret.littles.map(function(v) { return v.author });
  if (ret.somewhats) ret.somewhats = ret.somewhats.map(function(v) { return v.author });
  if (ret.verys) ret.verys = ret.verys.map(function(v) { return v.author });
}
...
LawSchema.virtual('nothings').get(function() {
  if (!this.scores) return;
  return this.scores.filter(function(v) {
    return "nothing" === v.value;
  });
});
...
LawSchema.virtual('littles').get(function() {
  if (!this.scores) return;
  return this.scores.filter(function(v) {
    return "little" === v.value;
  });
});

/**
 * Get `somewhats` scores
 *
 * @return {Array} scorers
 * @api public
 */

LawSchema.virtual('somewhats').get(function() {
  if (!this.scores) return;
  return this.scores.filter(function(v) {
    return "somewhat" === v.value;
  });
});
...
LawSchema.virtual('verys').get(function() {
  if (!this.scores) return;
  return this.scores.filter(function(v) {
    return "very" === v.value;
  });
...
/**
 * Score Law with provided citizen
 * and scoring value
 *
 * @param {Citizen|ObjectId|String} citizen
 * @param {String} value
...
LawSchema.methods.score = function(citizen, value, cb) {
  if ('recount' === this.status) return cb(new Error('Voting is closed on recount.'));
  if ('closed' === this.status) return cb(new Error('Voting is closed.'));
  // Here we could provide a 5000ms tolerance (5s)
  // or something... to prevent false positives
  ...
  var score = { author: citizen, value: value, caster: citizen };
  this.unscore(citizen);
  this.scores.push(score);
  // Add citizen as participant
  this.addScoreParticipant(citizen);
  this.save(cb);
};
...
LawSchema.methods.addScoreParticipant = function(citizen, cb) {
  this.scoreParticipants.addToSet(citizen);
  if (cb) this.save(cb);
}
...
LawSchema.methods.unvote = function(citizen, cb) {
  ...
  if (voted.length) this.save(cb);
};

/**
 * Unscore Law from provided citizen
 *
 * @param {Citizen|ObjectId|String} citizen
 * @param {Function} cb
 * @api public
...
LawSchema.methods.unscore = function(citizen, cb) {
  var scores = this.scores;
  var c = citizen.get ? citizen.get('_id') : citizen;

  var scored = scores.filter(function(v) {
    var a = v.author.get ? v.author.get('_id') : v.author;
    return a.equals
      ? a.equals(c)
      : a === c;
  ...
  log('About to remove scores %j', scored);
  scored.length && scored.forEach(function(v) {
    var removed = scores.id(v.id).remove();
    log('Remove vote %j', removed);
  });
...
LawSchema.methods.scoredBy = function(citizen) {
  var scores = this.scores;
  var c = citizen.get ? citizen.get('_id') : citizen;
